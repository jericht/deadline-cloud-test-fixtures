...
from bealine_test_scaffolding.bealine_manager import BealineManager
...
class JobAttachmentManager:
    ...
    def __init__(self, stage: str, account_id: str):
        ...
        self.bealine_manager = BealineManager(should_add_bealine_models=True)
    ...
    def deploy_resources(self):
        ...
        """
        try:
            self.bealine_manager.create_kms_key()
            self.bealine_manager.create_farm("job_attachments_test_farm")
            self.bealine_manager.create_queue("job_attachments_test_queue")
            self.deploy_stack()
    ...
    def cleanup_resources(self):
        ...
        """
        self.bealine_manager.delete_additional_queues()
        if self.bealine_manager.queue_id:
            self.bealine_manager.delete_queue()
        if self.bealine_manager.farm_id:
            self.bealine_manager.delete_farm()
        if self.bealine_manager.kms_key_metadata:
            self.bealine_manager.delete_kms_key()
        self.empty_bucket()
