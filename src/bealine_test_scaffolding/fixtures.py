...
def bootstrap_test_resources(cfn_client: botocore.client.BaseClient) -> None:
    ...
    cfn_template: dict[str, Any] = {
        ...
        "Description": "Stack created by bealine-test-scaffolding",
        "Resources": {
            # A role for use by the Worker Agent after being bootstrapped
            BEALINE_WORKER_ROLE: generate_worker_role_cfn_template(),
            BEALINE_WORKER_BOOTSTRAP_ROLE: generate_boostrap_worker_role_cfn_template(),
            BEALINE_QUEUE_SESSION_ROLE: generate_queue_session_role(),
            BEALINE_WORKER_BOOSTRAP_INSTANCE_PROFILE_NAME: generate_boostrap_instance_profile_cfn_template(),
...
def bealine_manager_fixture():
    bealine_manager_fixture = BealineManager(should_add_bealine_models=True)
...
def worker_role_arn(iam_client: botocore.client.BaseClient) -> str:
    response = iam_client.get_role(RoleName=BEALINE_WORKER_ROLE)
    return response["Role"]["Arn"]


@pytest.fixture(scope="session")
def bealine_scaffolding(
    bealine_manager_fixture: BealineManager, worker_role_arn: str
) -> Generator[Any, None, None]:
    bealine_manager_fixture.create_scaffolding(worker_role_arn, JOB_ATTACHMENTS_BUCKET_NAME)
...
def create_worker_agent(
    bealine_scaffolding, launch_instance: str, send_ssm_command: Callable
) -> Generator[Any, None, None]:
    def configure_worker_agent_func() -> Dict:
        """Creates a Bealine Farm, starts an instance and configures and starts a Worker Agent."""
        assert bealine_scaffolding
        ...
        configuration_command_response = send_ssm_command(
            ...
            (
                ...
                f"touch /etc/sudoers.d/bealine-worker-job-user && \n"
                f'echo "agentuser ALL=(jobuser) NOPASSWD:ALL" | sudo tee /etc/sudoers.d/bealine-worker-job-user && \n'
                f"python3.9 -m venv /opt/bealine/worker && \n"
                f"source /opt/bealine/worker/bin/activate && \n"
                f"pip install --upgrade pip && \n"
                f"touch /opt/bealine/worker/pip.conf && \n"
                ...
                f"aws s3 cp s3://{BEALINE_SERVICE_MODEL_BUCKET}/service-2.json /tmp/bealine-beta-2020-08-21.json && \n"
                f"chmod +r /tmp/bealine-beta-2020-08-21.json && \n"
                f"sudo -u agentuser aws configure add-model  --service-model file:///tmp/bealine-beta-2020-08-21.json --service-name bealine && \n"
                f"mkdir /var/lib/bealine /var/log/amazon/bealine/ && \n"
                f"chown agentuser:agentuser /var/lib/bealine /var/log/amazon/bealine/ && \n"
                f"pip install bealine-worker-agent && \n"
                f"sudo -u agentuser /opt/bealine/worker/bin/bealine_worker_agent --help"
            ),
        )
    ...
    def start_worker_agent_func() -> Dict:
        start_command_response = send_ssm_command(
            launch_instance,
            (
                f"nohup sudo -E AWS_DEFAULT_REGION=us-west-2 -u agentuser /opt/bealine/worker/bin/bealine_worker_agent --farm-id {bealine_scaffolding.farm_id} --fleet-id {bealine_scaffolding.fleet_id} --allow-instance-profile >/dev/null 2>&1 &"
            ),
        )
